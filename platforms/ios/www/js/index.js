...
function onDeviceReady() {
  ...
  if(localStorage.imageData) { 
    ctx.putImageData(localStorage, 0, 0);
  }
...
.then(function(name) {     
  ...
  notes.innerHTML+='<ons-list-item id="'+id+'" onclick="setOrder(this)"><ons-checkbox onclick="strike(this)"></ons-checkbox><span>'+name+'</span><span class="edit-note" onclick="editNote(this)"><i class="zmdi zmdi-edit"></i></span><span class="del-note" onclick="delNote(this)">X</span></ons-list-item>';
  localStorage.notes=  notes.innerHTML;
  checkBadges();
...
function checkBadgesReminders() {
  ...
  remindersMenu.setAttribute("badge", numberBadgesReminders);
}
